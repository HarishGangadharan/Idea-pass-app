...
export default class SelectComponent extends BaseComponent {
  static schema(...extend) {
    return BaseComponent.schema({
      ...
      baseTemplate: '<span>{{ item.name }}</span>',
      selectFields: '',
      searchThreshold: 0.3,
      fuseOptions: {},
      customOptions: {}
  ...
  itemTemplate(data) {
    ...
    if (this.component.key === 'data.resource') {
      template = this.interpolate(template);
    }
    template = template ? this.interpolate(template, { item: data }) : data.label;
    if (template) {
    ...
    if (_.isObject(data)) {
      if (this.component.valueProperty) {
        if (this.component.key === 'data.resource') {
          ...
          valueProperty = this.interpolate(valueProperty).replace(/<\/?[^>]+(>|$)|[ {}]/g, '').split('item.')[1];
          return _.get(data, valueProperty ? valueProperty : this.component.valueProperty);
        }
  ...
  setItems(items, fromSearch) {
    ...
    if (this.component.selectValues) {
      items = _.get(items, this.interpolate(this.component.selectValues));
    }
...
SelectComponent.editForm = selectForm;
// Register the component to the Formio.Components registry.
Components.addComponent('select', SelectComponent);
